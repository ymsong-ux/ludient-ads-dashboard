...
def search_query_report():
    ...
    rows = [
        # (우리 키워드, 실제 검색어, 노출, 클릭, 구매, 광고비)
        ("PDRN 크림", "PDRN 크림 효과", 234, 8, 1, 9_600),
        ("PDRN 크림", "PDRN 크림 가격", 156, 12, 0, 14_400),
        ("PDRN 크림", "PDRN 크림 추천", 89, 3, 0, 3_600),
        ("PDRN 크림", "PDRN 크림 무료배송", 42, 5, 0, 6_000),
        ("PDRN 크림", "PDRN 크림 사용법", 38, 2, 0, 2_400),
        ("PDRN 크림", "PDRN 크림 후기", 71, 6, 1, 7_200),
        ("PDRN 크림", "PDRN 크림 부작용", 28, 1, 0, 1_200),
        ("재생 크림", "재생 크림 추천 더마", 178, 15, 2, 33_000),
        ("재생 크림", "재생 크림 비교", 92, 3, 0, 6_600),
        ("재생 크림", "재생 크림 무료배송", 65, 8, 0, 17_600),
        ("재생 크림", "재생 크림 30대", 54, 4, 1, 8_800),
        ("재생 크림", "재생 크림 순위", 41, 2, 0, 4_400),
        ("시술 후 크림", "시술 후 크림 진정", 88, 9, 2, 7_200),
        ("시술 후 크림", "시술 후 크림 추천", 67, 5, 1, 4_000),
        ("시술 후 크림", "레이저 후 크림", 52, 7, 1, 5_600),
        ("시술 후 크림", "필러 후 크림", 31, 2, 0, 1_600),
        ("안티에이징 크림", "안티에이징 크림 추천", 124, 4, 0, 7_200),
        ("안티에이징 크림", "40대 안티에이징", 88, 2, 0, 3_600),
        ("안티에이징 크림", "안티에이징 크림 비교", 56, 1, 0, 1_800),
        ("안티에이징 크림", "안티에이징 크림 50대", 42, 3, 1, 5_400),
        ("모공 앰플", "모공 앰플 추천", 76, 6, 1, 6_600),
        ("모공 앰플", "모공 줄이는법", 92, 4, 0, 4_400),
        ("모공 앰플", "모공 앰플 효과", 58, 3, 0, 3_300),
        ("모공 앰플", "코 모공 앰플", 45, 2, 0, 2_200),
    ]
    ...
    out = []
    for our_kw, query, imp, clk, purchases, spend in rows:
